...
class WyleCLI {
  ...
  async selectTemplate() {
    ...
    templates.forEach((template, index) => {
      console.log(
        `  ${chalk.gray(`${index + 1}.`)} ${chalk[template.color]('●')} ${chalk.bold(template.name)} - ${template.description}`
      );
    });
    console.log('');
    ...
    return new Promise((resolve) => {
      const askTemplate = () => {
        rl.question(chalk.cyan('请输入模板编号或名称 (1-' + templates.length + '): '), (answer) => {
          ...
          if (!isNaN(choice) && choice >= 1 && choice <= templates.length) {
            rl.close();
            resolve(templates[choice - 1].name);
            return;
          }
          
          // 检查是否是名称选择
          const selectedTemplate = templates.find(t => 
            t.name.toLowerCase() === trimmedAnswer.toLowerCase()
          );
          ...
          if (selectedTemplate) {
            ...
            console.log(chalk.red(`❌ 无效选择 '${trimmedAnswer}'，请重新输入`));
            console.log(chalk.gray('提示：可以输入数字编号或模板名称'));
            askTemplate();
          }
        });
      };
  ...
  async createProject(projectName, template = null) {
    ...
    // 如果没有指定模板，则交互式选择
    if (!template) {
      console.log(chalk.cyan(`\n🎯 准备为项目 ${chalk.bold(projectName)} 选择模板...`));
